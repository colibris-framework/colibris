...
from colibris.paginators import PageNumberPaginator
...
class TestList:
    ...
    async def test_get_pagination_default_page_and_size(self, database, http_client):
        ...
        response = await http_client.request(hdrs.METH_GET, "/paginated-items")
        assert response.status == 200
    ...
    async def test_get_pagination_different_page(self, database, http_client):
        ...
        response = await http_client.request(hdrs.METH_GET, "/paginated-items", params={'page_size': 1, 'page': 1})
        assert response.status == 200
        ...
        response = await http_client.request(hdrs.METH_GET, "/paginated-items", params={'page_size': 1, 'page': 2})
        assert response.status == 200
        data = await response.json()
        assert data['results'][0]['name'] == second_item_name

        response = await http_client.request(hdrs.METH_GET, "/paginated-items", params={'page_size': 1, 'page': 3})
        assert response.status == 200
        data = await response.json()
        assert data['results'][0]['name'] == first_item_name

        response = await http_client.request(hdrs.METH_GET, "/paginated-items", params={'page_size': 1, 'page': 4})
        assert response.status == 200
        data = await response.json()
        assert data['results'] == []
